...
def genDAGInstance():
    ...
    num_cnt = random.randint(base_num_cnt + 1, 25)
    base_nums = [random.randint(1, 100) for _ in range(base_num_cnt)]

    texts = ["已知 {} .".format(
        " ".join([f"[v{i}] 等于 {x}" for i, x in enumerate(base_nums)])
    )]
...
for path, num in zip([train_path, dev_path, test_path], [10000, 1000, 1000]):
    data = [genDAGInstance() for _ in tqdm(range(num), total=num)]

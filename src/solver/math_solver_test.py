    ...
    def prepare_input(
        ...
        for ids in input_dict.input_ids:
            ...
            for i, token_id in enumerate(ids):
                ...
                if token_id not in quant_used and token_id in self.quant_tokens_id:
                    quant_ids.append(i)
                    quant_used.add(token_id)
    ...
    def expr_beam_search(
        ...
        while len(beams) > 0:
            ...
            for beam in next_beams:
                ...
                # if len(tokens) >= max_length and tokens[-1] not in [self.expr_tok.bos_token, self.expr_tok.eos_token]:
                #     continue

...
class Seq2seqSolver:
    ...
    @torch.no_grad()
    def batch_generate(self, batch_tokens: List[List[Tok]], max_length: int = 30) -> List[str]:
        self.eval()

        BS = len(batch_tokens)

        input_dict, nums_ids = self.prepare_input(batch_tokens)
        text_embedding, encoder_outputs, W_nums, output_mask = self.encode(input_dict, nums_ids)
        past_value = text_embedding  # (BS, H)
        decoder_input_ids = torch.tensor(
            self.dec_tokenizer.bos_token_id, 
            dtype=torch.long,
            device=past_value.device
        ).view(1, 1).repeat(BS, 1)

        predict_ids = [[] for _ in range(BS)]

        length = 0
        end_cnt = 0
        end_mask = [0 for _ in range(BS)]
        
        while length < max_length and end_cnt < BS:
            next_token_logits, past_value = self.decode(
                decoder_input_ids=decoder_input_ids[:, -1],
                past_value=past_value,
                W_nums=W_nums,
            ...
            next_token_id = torch.argmax(next_token_logits, dim=1, keepdim=True)  # [BS, 1]
            
            for i in range(BS):
                if end_mask[i]:
                    continue
                next_id = next_token_id[i, 0].item()
                if next_id == self.dec_tokenizer.eos_token_id:
                    end_cnt += 1
                    end_mask[i] = 1
                else:
                    predict_ids[i].append(next_id)
            
            decoder_input_ids = torch.cat((decoder_input_ids, next_token_id), dim=1)  # [BS, L] -> [BS, L + 1]
            length += 1
        
        predict_tokens = [self.dec_tokenizer.convert_ids_to_tokens(predict_ids[i]) for i in range(BS)] 
        predict_text = ["".join(predict_tokens[i]) for i in range(BS)]
        return predict_text

...
class MathSolver(nn.Module):
    ...
    def __init__(
        ...
        self.tok = model_dict["tokenizer"][model_name].from_pretrained(
            self.cfg.model_name,
            cache_dir="../cache/model"
        )
    ...
    def update_voc(self, const_nums_size: int):
        ...
        self.eos0_token = self.g_tokens[0]
        self.eos1_token = self.g_tokens[1]
        self.voc_eos0_token_id = self.tok.convert_tokens_to_ids(self.eos0_token)
        self.voc_eos1_token_id = self.tok.convert_tokens_to_ids(self.eos1_token)
    ...
    def prepare_output(
        ...
        shift_labels[:, 0] = self.dvoc_eos0_token_id # self.dvoc_token_id["[eos0]"]
    ...
    def forward(self, batch: List[MathDataInstance]) -> Tensor:
        ...
        shift_labels, labels = self.prepare_output(
            [I.parse_output(self.eos0_token, self.eos1_token) for I in batch]
        )
        ...
        with torch.no_grad():
            ...
            # for i in range(B):
            #     for j in range(L):
            #         if labels[i, j].item() != -1 and preds[i, j].item() != labels[i, j].item():
            #             tgt.append(self.dvoc2voc[labels[i, j].item()])
            #             our.append(self.dvoc2voc[preds[i, j].item()])
            # if Acc.item() > 0.90:
            #     print("tgt:", self.tok.convert_ids_to_tokens(tgt))
            #     print("our:", self.tok.convert_ids_to_tokens(our))
    ...
    def generate(
        ...
        while len(nums) + len(expr_list) < self.cfg.max_nums_size and step < self.cfg.max_step_size:
            ...
            op_text, exit_flag, past_value = self.generate_expr(
                I.parse_input(self.tok.sep_token),
                prev_past_value=prev_past_value,
            )
            try:
                expr = self.parse_expr(op_text, nums_size)
    ...
    def expr_beam_search(
        ...
        decoder_input_id = torch.tensor(
            self.dvoc_eos0_token_id, # self.dvoc_token_id["[eos0]"]
            dtype=torch.long,
        ...
        while len(beams) > 0:
            ...
            for beam in next_beams:
                ...
                if not grammar_test(tokens, max_length, self.eos0_token, self.eos1_token):
                    continue
                ...
                if self.cfg.dataset_name == "svamp" and not svamp_constraint_test(tokens):  # hard constraint for svamp
                    continue
                if not beam.end and tokens[-1] in [self.eos0_token, self.eos1_token]:
                    beam.end = True
    ...
    def beam_search(
        ...
        beams: List[StatBeam] = [StatBeam([], None, 0.0, self.eos0_token)]
        while len(beams) > 0:
            do_search = False
            for beam in beams:
                do_search |= (not beam.end)
...
def grammar_test(tokens: List[Tok], max_length: int, eos0_token: str, eos1_token: str) -> bool:
    if len(tokens) >= max_length and tokens[-1] not in [eos0_token, eos1_token]:
        return False
    end = tokens[-1] in [eos0_token, eos1_token]
